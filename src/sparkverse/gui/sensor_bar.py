        ...
        if SensorBar.sensor_count>0:
            for i in range(SensorBar.sensor_count+1):
                if i==SensorBar.last_key-1:
                    cv2.rectangle(img=image,pt1=(10,30+i*40),pt2=(40,60+i*40),color=(255,0,255),thickness=3)#pylint: disable=E1101
                    cv2.putText(img=image,text=SensorBar.sensor_key[i],org=(20,50+i*40),fontFace=SensorBar.font,fontScale=0.5,color=(255,0,255),thickness=2)#pylint: disable=E1101
                    cv2.putText(img=image,text=SensorBar.sensor_description[i],org=(50,50+i*40),fontFace=SensorBar.font,fontScale=0.5,color=(255,0,255),thickness=2)#pylint: disable=E1101
                else:
